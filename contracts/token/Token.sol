...
contract Token is IToken, AgentRoleUpgradeable, TokenStorage {
    ...
    function batchMint(
        address[] calldata _toList,
        uint256[] calldata _amounts,
        string[] calldata _metadataUriList ) external override {
        for (uint256 i = 0; i < _toList.length; i++) {
            mint(_toList[i], _amounts[i], _metadataUriList[i]);
        }
    }
    ...
    function getTokenMetadata(uint256 _tokenId) external view returns (string memory) {
        ...
        return _tokenMetadata[_tokenId].uri;
    }    
    ...
    function mint(
        ...
        _mint(_to, _amount, _metadataUri);
    ...
    function _mint(
        ...
        // Store the metadata
        _tokenMetadata[tokenId] = TokenMetadata({ uri: _metadataUri });
