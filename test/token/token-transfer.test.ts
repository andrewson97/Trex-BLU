  ...
  describe('.mint', () => {
    describe('when sender is not an agent', () => {
      it('should revert', async () => {
        ...
        await expect(token.connect(aliceWallet).mint(aliceWallet.address, 100, "Token-metadata-Uri-comes-here")).to.be.revertedWith('AgentRole: caller does not have the Agent role');
      });
    });
    ...
    describe('when recipient identity is not verified', () => {
      it('should revert', async () => {
        ...
        await expect(token.connect(tokenAgent).mint(anotherWallet.address, 100, "Token-metadata-Uri-comes-here")).to.be.revertedWith('Identity is not verified.');
      });
    });
    ...
    describe('when the mint breaks compliance rules', () => {
      it('should revert', async () => {
        ...
        await expect(token.connect(tokenAgent).mint(aliceWallet.address, 100, "Token-metadata-Uri-comes-here")).to.be.revertedWith('Compliance not followed');
      });
    });
  });
