...
describe('Token - Information', () => {
  ...
  describe('.getTokenMetadata()', () => {
    it('should return the correct token metadata', async () => {
      const {
        suite: { token },
      } = await loadFixture(deployFullSuiteFixture);

      const tokenId = 1;
      const expectedUri = 'https://token-metadata.example.com/1';

      // Assume that token metadata is set elsewhere in the contract setup
      await token.getTokenMetadata(tokenId);

      const uri = await token.getTokenMetadata(tokenId);
    ...
    it('should revert if token ID does not exist', async () => {
      ...
      await expect(token.getTokenMetadata(nonExistentTokenId)).to.be.revertedWith('Token ID does not exist');
    });
  });
});
