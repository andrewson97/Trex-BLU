...
export async function deployFullSuiteFixture() {
  ...
  await token.connect(tokenAgent).mint(aliceWallet.address, 1000, "Token-metadata-Uri-comes-here");
  await token.connect(tokenAgent).mint(bobWallet.address, 500, "Token-metadata-Uri-comes-here");
