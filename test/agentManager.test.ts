  ...
  describe('.callMint', () => {
    describe('when specified identity is missing the SupplyModifier role', () => {
      it('Should revert', async () => {
        ...
        await expect(agentManager.connect(aliceWallet).callMint(bobWallet.address, 1000, "Token-metadata-Uri-comes-here", aliceIdentity.address)).to.be.revertedWith(
    ...
    describe('when specified identity has the SupplyModifier role but the sender is not authorized for it', () => {
      it('should revert', async () => {
        ...
        await expect(agentManager.connect(anotherWallet).callMint(bobWallet.address, 1000, "Token-metadata-Uri-comes-here", aliceIdentity.address)).to.be.revertedWith(
          'Role: Sender is NOT Supply Modifier',
        );
      });
    });
    ...
    describe('when identity has the SupplyModifier role and the sender is authorized for it', () => {
      it('Should perform the mint', async () => {
        ...
        const mintTx = await agentManager.connect(aliceWallet).callMint(bobWallet.address, 1000, "Token-metadata-Uri-comes-here", aliceIdentity.address);
  ...
  describe('.callBatchMint', () => {
    describe('when specified identity is missing the SupplyModifier role', () => {
      it('Should revert', async () => {
        ...
        await expect(
          agentManager.connect(aliceWallet).callBatchMint([bobWallet.address, aliceWallet.address], [1000, 500],[ "Token-metadata-Uri-comes-here", "Token-metadata-Uri-comes-here"], aliceIdentity.address),
    ...
    describe('when specified identity has the SupplyModifier role but the sender is not authorized for it', () => {
      it('should revert', async () => {
        ...
        await expect(
          agentManager.connect(anotherWallet).callBatchMint([bobWallet.address, aliceWallet.address], [1000, 500],[ "Token-metadata-Uri-comes-here", "Token-metadata-Uri-comes-here"], aliceIdentity.address),
        ).to.be.revertedWith('Role: Sender is NOT Supply Modifier');
      });
    });
    ...
    describe('when identity has the SupplyModifier role and the sender is authorized for it', () => {
      it('Should perform the batch mint', async () => {
        ...
        const mintTx = await agentManager
          .connect(aliceWallet)
          .callBatchMint([bobWallet.address, aliceWallet.address], [1000, 500],[ "Token-metadata-Uri-comes-here", "Token-metadata-Uri-comes-here"], aliceIdentity.address);
